...
function setUpTextures(){
    ...
    // load the actual image
    var myImage = document.getElementById ('night-world-texture');
    myImage.crossOrigin = "";
    ...
    //----------------------------------------------------------------------
    // get some texture space from the gpu
    myProceduralTexture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, myProceduralTexture);
    
    // // // load the actual image
    // var myProceduralImage = document.getElementById ('night-world-texture');
    // myImage.crossOrigin = "";
...
function drawCurrentShape () {
    ...
    //----------------------------------------------------------------------
    // set up the new myImagetexture uniform & other uniforms that you might
    // have added to the shader -- created by Isaias
    ...
    if (curTexture === "proc"){
      gl.activeTexture (gl.TEXTURE0);
      gl.bindTexture (gl.TEXTURE_2D, myProceduralTexture);
      gl.uniform1i (program.uMyProceduralTexture, 0);
...
function initProgram (vertexid, fragmentid) {
  ...
  //New Additionally Reference, this is the new uniform variable for the texture associated with MyImageTexture
  program.uMyImageTexture = gl.getUniformLocation(program, "MyImageTexture");
  program.uMyProceduralTexture = gl.getUniformLocation(program, "MyProceduralTexture");
...
// general call to make and binfd a new object based on current
...
function bindVAO (shape, program) {
  ...
  function init() {
    ...
    sphereGlobeProgram = initProgram('sphereMap-V', 'sphereMap-NightEarth');
